...
public class ObjectManager {
		...
		for (GameContainer gc : containers) {
			if (gc.isActive() && !gc.doAnimation) {
				if (gc.getHitbox().intersects(attackBox)) {
					...
					if (rate.nextInt(0, 100) >= 20)
						potions.add(new Potion((int) (gc.getHitbox().x + gc.getHitbox().width / 2),
								(int) (gc.getHitbox().y + gc.getHitbox().height / 4), type));
					return;
				}
			}
		}
	...
	private void loadImgs() {
		...
		for (int i = 0; i < cannonsImgs.length; i++) {
			cannonsImgs[i] = temp.getSubimage(i * 40, 0, 40, 26);
		}

		canonBallImg = LoadSave.GetSpriteAtlas(LoadSave.CANNON_BALL);
	}
	...
	private boolean isPlayerInRange(Cannon c, Player player) {
		...
		return absValue <= Game.TILE_SIZE * 6;
	}
	...
	private void drawProjectiles(Graphics g, int xLvlOffset) {
		for (Projectile p : projectiles) {
			if (p.isActive()) {
				g.drawImage(canonBallImg, (int) (p.getHitbox().x - xLvlOffset), (int) (p.getHitbox().y),
	...
	private void drawCannons(Graphics g, int xLvlOffset) {
		for (Cannon c : cannons) {
			...
			g.drawImage(cannonsImgs[c.getAniIndex()], x, (int) (c.getHitbox().y), width, CANNON_HEIGHT, null);
		}
		...
		for (GameContainer c : containers) {
			if (c.isActive()) {
				...
				g.drawImage(containerImgs[type][c.getAniIndex()],
						...
						(int) (c.getHitbox().y - c.getyDrawOffset()), CONTAINER_WIDTH, CONTAINER_HEIGHT, null);
			}
		}
	}
